layui.use('common', function(){
  ...
  function initMenu(){
     $.ajax({  
         url: "/management/v1/permissions/current",
         type:"get",  
         async:false,
  ...
  function showLoginInfo(){
    $.ajax({
      type : 'get',
      url : '/management/v1/users/self/queryInfo',
      async : false,
      success : function(data) {
        $(".admin-header-user span").text(data.content.nickname);
        sessionStorage.setItem('operator',data.content.username)
        var pro = window.location.protocol;
        var host = window.location.host;
        var domain = pro + "//" + host;
        
        var sex = data.content.sex;
        var url = data.content.headImgUrl;
        ...
        if(url == null || url == ""){
          ...
          $.getFileUrl('driver',data.content.headImgUrl,function(status,option){
            if(status == 'success')
            {               
              img.attr("src", option);
            }
